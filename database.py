...
from utils import normalize_url, normalize_text, calculate_job_similarity, parse_salary_etb
...
CATEGORY_KEYWORDS = {
    ...
    "engineering": [
        ...
        "construction", "architect", "surveyor", "structural", "site engineer",
    ],
    "health": [
        "nurse", "doctor", "medical", "pharmacy", "pharmacist", "hospital",
        "clinical", "lab technician", "midwife", "health officer", "physician",
    ],
    "teaching": [
        "teacher", "lecturer", "instructor", "professor", "tutor", "trainer",
        "academic", "teaching",
    ],
    "marketing": [
        "marketing", "sales officer", "sales manager", "brand", "digital marketing",
        "social media", "advertising", "business development", "sales representative",
    ],
    "admin": [
        "assistant", "secretary", "administrative", "clerk", "receptionist",
        "coordinator", "executive assistant", "office manager", "admin officer",
    ],
    "logistics": [
        "logistics", "supply chain", "warehouse", "inventory", "procurement",
        "store keeper", "dispatch", "fleet",
    ],
    "hr": [
        "human resource", "hr officer", "hr manager", "recruitment", "talent",
        "payroll", "personnel", "training officer",
    ],
    "government": [
        "ministry", "authority", "government", "public service", "civil service",
        "federal",
    ],
}
...
def _jobs_base_where(
    ...
    if min_salary and min_salary > 0:
        conditions.append("salary_min_etb IS NOT NULL AND salary_min_etb >= %s")
...
class Database:
    ...
    def save_job(self, job_data: Dict[str, Any]) -> Optional[int]:
        ...
        salary_min_etb = parse_salary_etb(job_data.get("salary") or "")
        ...
        if duplicate:
            dup_channel = duplicate.get('telegram_channel', '')
            cross = f" (cross-channel: {dup_channel} → {telegram_channel})" if dup_channel and telegram_channel and dup_channel != telegram_channel else ""
            logger.info(f"Duplicate detected: #{duplicate.get('id')} '{title}'{cross}, merging...")
        ...
        params = {
            ...
            'deadline': _strip_md(job_data.get('deadline')),
            'salary': _strip_md(job_data.get('salary')),
            'salary_min_etb': salary_min_etb,
                ...
                if existing:
                    cur.execute("""
                        UPDATE jobs SET
                            ...
                            deadline_text = COALESCE(%s, deadline_text),
                            salary = COALESCE(%s, salary),
                            salary_min_etb = COALESCE(%s, salary_min_etb),
                            scraped_data = %s,
                            telegram_id = %s,
                            telegram_channel = %s,
                            telegram_date = COALESCE(%s, telegram_date),
                    ...
                    """, (
                        ...
                        params['salary'], params.get('salary_min_etb'),
                        params['scraped_data'],
                    ...
                    cur.execute("""
                        INSERT INTO jobs (
                            ...
                            deadline_text, salary, salary_min_etb, scraped_data, raw_text,
    ...
    def get_jobs(
        ...
        try:
            conditions, params = _jobs_base_where(search=search, category=category, location=location, min_salary=min_salary)
    ...
    def get_job_stats(self, job_id: int) -> Dict[str, Any]:
        """Return per-job view + apply counters used for trust signals on the modal."""
        self._ensure_connection()
        out = {
            "views_total": 0,
            "views_24h": 0,
            "apply_clicks_total": 0,
            "apply_clicks_24h": 0,
            "created_at": None,
        }
        try:
            with self.conn.cursor() as cur:
                ...
                out["views_total"] = int(cur.fetchone()[0] or 0)
                cur.execute(
                    "SELECT COUNT(*) FROM job_views WHERE job_id = %s AND viewed_at > NOW() - INTERVAL '24 hours'",
                    (job_id,),
                )
                out["views_24h"] = int(cur.fetchone()[0] or 0)
                cur.execute(
                    "SELECT COUNT(*) FROM apply_clicks WHERE job_id = %s",
                    (job_id,),
                )
                out["apply_clicks_total"] = int(cur.fetchone()[0] or 0)
                cur.execute(
                    "SELECT COUNT(*) FROM apply_clicks WHERE job_id = %s AND clicked_at > NOW() - INTERVAL '24 hours'",
                    (job_id,),
                )
                out["apply_clicks_24h"] = int(cur.fetchone()[0] or 0)
                cur.execute(
                ...
                if row and row[0] is not None:
                    val = row[0]
                    out["created_at"] = val.isoformat() if hasattr(val, "isoformat") else str(val)
        except Exception as e:
            logger.warning("get_job_stats failed: %s", e)
            try:
                self.conn.rollback()
            except Exception:
                pass
    ...
    def get_job_ids_for_sitemap(self, limit: int = 5000) -> List[Dict[str, Any]]:
        """Return id + created_at for all active jobs, newest first. Used by /sitemap.xml."""
        self._ensure_connection()
        try:
            with self.conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(
                    "SELECT id, created_at FROM jobs ORDER BY id DESC LIMIT %s",
                    (limit,),
                )
                return [dict(r) for r in cur.fetchall()]
        except Exception as e:
            logger.warning("get_job_ids_for_sitemap failed: %s", e)
            return []
