...
# Default = gemini only. Ollama/Groq are opt-in via AI_PROVIDERS env var
# because checking them when they aren't configured wastes seconds per job.
AI_PROVIDERS = os.getenv("AI_PROVIDERS", "gemini").strip()
...
def _get_fallback_provider() -> FallbackAIProvider:
    ...
    for name in provider_names:
        try:
            ...
            logger.warning("Skipping %s provider — missing dependency: %s", name, e)
        except Exception as e:
            logger.warning("Failed to initialize %s provider: %s", name, e)

    if not providers:
        logger.error("No AI providers configured. Set AI_PROVIDERS and the matching API key in .env")
    else:
        available = [p.name for p in providers if p.is_available()]
        if available:
            logger.info("AI normalization enabled. Provider chain: %s", available)
        else:
            logger.error(
                "AI providers configured (%s) but none are available. "
                "Check API keys and network.", [p.name for p in providers]
            )
