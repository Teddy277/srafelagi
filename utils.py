...
_SALARY_NON_NUMERIC = re.compile(
    r'\b(negotiable|competitive|tbd|to be determined|as per scale|'
    r'based on experience|attractive|n/?a|not specified|open|'
    r'according to)\b',
    re.IGNORECASE,
)

# Approximate exchange rates for non-ETB currencies → ETB
_FX = {"usd": 55, "dollar": 55, "$": 55, "eur": 60, "€": 60, "gbp": 70}


def parse_salary_etb(text: str) -> Optional[int]:
    """
    Parse a free-text salary string into an estimated minimum ETB value.

    Handles: "15,000 ETB/month", "8k-12k birr", "1,500 USD", "20,000 ብር"
    Returns None for "Negotiable", non-numeric strings, or implausible values.
    """
    if not text:
        return None
    text = text.strip()
    if _SALARY_NON_NUMERIC.search(text):
        return None
    ...
    # Detect currency
    multiplier = 1
    for token, rate in _FX.items():
        if token in tl:
            multiplier = rate
            break  # first match wins

    # Extract all number+optional-K patterns
    amounts = []
    for num_str, k_suffix in re.findall(r'(\d[\d,]*(?:\.\d+)?)\s*([kK]?)', text):
        if not num_str:
            continue
        try:
    ...
    min_amount = min(a for a in amounts if a > 0) * multiplier

    # Sanity check: plausible Ethiopian salary (500–500,000 ETB/month)
    if not (500 <= min_amount <= 500_000):
        return None
